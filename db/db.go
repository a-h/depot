...
import (
	...
	hash "github.com/nix-community/go-nix/pkg/nixhash"
...
func (d *DB) QueryPathInfo(ctx context.Context, storePath string) (pathInfo PathInfo, ok bool, err error) {
	...
	switch hashPrefix {
	case "sha1":
		hashType = nixhash.SHA1
	case "sha256":
		hashType = nixhash.SHA256
	case "sha512":
		hashType = nixhash.SHA512
	default:
	...
	nixHash, err := hash.NewHash(hashType, sum)
	if err != nil {
		return pathInfo, false, fmt.Errorf("db: failed to parse hash %q: %w", pathInfo.Hash, err)
	}
	pathInfo.Hash = nixHash.Format(nixhash.NixBase32, true)
