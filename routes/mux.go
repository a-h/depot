...
func New(log *slog.Logger, nixdb *nixdb.DB, npmdb *npmdb.DB, pythondb *pythondb.DB, storePath string, authConfig *auth.AuthConfig, privateKey *signature.SecretKey) http.Handler {
	...
	pythonh := pythonhandler.New(log, pythondb, pythonStorage, "http://localhost:8080/python")
